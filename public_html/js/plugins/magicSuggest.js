  ...
  this.removeFromSelection = function(items, isSilent) {
    ...
    $.each(items, function(index, json) {
      var value = ms.getValue()
        ...
        , i = $.inArray(json[cfg.valueField], arrayValue);
      if (i > -1) {
  ...
  */
  _renderSelection: function() {
    ...
    $.each(_selection, function (index, value) {
      ...
      if ($.isFunction(cfg.element_extra)) {
        cfg.element_extra(index, value).appendTo(selectedItemEl);
      } else {
        $(cfg.element_extra).each (function (){
        /*formato de nombre que ocupara nuestro nuevo elemento*/
        if(this.name==undefined){
          this.name="";
        }

        if(this.name){
             var name=this.name+"["+index +"]["+this.id+"]";
        }
        else{
            var name=null;
        }

        //var name = this.name || this.name+"["+index +"]["+this.id+"]";
          var options_input={name:name,type:this.type,value:value[this.id]};
          if(this.type=="select"){
              var tag_input="<select/>";
             options_input['class'] =("ms-select-select-item btn-mini");
          }
          else{
            options_input['class'] =("ms-input-select-item");
            var tag_input="<input/>";
          }
          var $input_ex= $(tag_input, options_input);

           if(this.type=="select"){
                var options_ex=this.options == undefined ? [] : this.options ;
                var options_html="";

                for (  var key in  options_ex ){
                  options_html+="<option value='"+key+"'> " + options_ex[key]+"</option>";
                }


                $input_ex.append(options_html);
          }
          $input_ex.val(value[this.id]);
          var data_=this;
          $input_ex.data("json",value).appendTo(selectedItemEl);
          $input_ex.on("change",function (){
              _selection[index][data_.id]=$(this).val();
          });
        });
      }
